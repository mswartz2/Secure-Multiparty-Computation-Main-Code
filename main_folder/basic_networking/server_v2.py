...
class Server(SmpcNode):
    ...
    def get_gn_sn_for_client(self, ids, shares):
        """Receives the x value and share for 2 x-values
        Returns:
        - array of g(ids[:-1]) for all points the server has
        - nested array of s(id_n) for all ids for all points the server has
        - array of labels"""
