import numpy as np
import random
from basic_networking.smpc_node_test1degree2shares import SmpcNode
from basic_networking.server_test1degree2shares import Server
...
class Client(SmpcNode):
    ...
    def get_distances_and_labels(
        ...
        id_1, id_2 = random.Random(rand_seed).sample(range(1, rand_range), 2)
        ...
        g_1_arr, s_2_arr, label_arr = serverNode.get_gn_sn_for_client(id_1, id_2, f_2)
        ...
        # calculate s_0_arr
        distance_labels_arr = []
        for s_1, s_2, label in zip(s_1_arr, s_2_arr, label_arr):
            s_0 = self._reconstruct([s_1, s_2], [id_1, id_2])
            try:
                distance = np.sqrt(abs(s_0))
            except:
                distance = 0
